...
impl<'a> UiProcessor<'a> {
    ...
    pub fn update_gyph_cache(
        &mut self,
        glyphs: impl Iterator<Item = (FontId, cc::text::PositionedGlyph)>,
    ) {
        ...
        while let Err(_) = {
            ...
            self.glyph_cache.cache_queued(|region, data| {
                ...
                for row in region.min.y..region.max.y {
                    &mut cache_data[(row * width + region.min.x) as usize
                        ..(row * width + region.max.x) as usize]
        ...
        info!(
            ...
            gc,
            gl.iter()
                .map(|&c| TryInto::<char>::try_into(c).unwrap())
                .collect::<String>()
        );
            ...
            match primitive.kind {
                ...
                PrimitiveKind::Text {
                    ...
                    for glyph in text.positioned_glyphs(1.0 /* dpi_factor */) {
                        let result = self
                            .glyph_cache
                            .rect_for(font_id.index(), glyph)
                            .map_err(|_| ConBackError::UncachedGlyph(glyph.id().0))?;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        while let Some(prim) = self.prims.next() {
            if let PrimitiveKind::Text { text, font_id, .. } = prim.kind {
                self.current
                    .extend_from_slice(text.positioned_glyphs(self.dpi_factor));
