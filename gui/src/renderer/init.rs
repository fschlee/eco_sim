...
pub trait InstSurface {
    ...
    fn get_mut_surface<'a>(
        &'a mut self,
    ) -> &'a mut <<Self as InstSurface>::Back as Backend>::Surface;
    ...
    )
{
    ...
    fn create(name: &str, version: u32, window: &Window) -> Self {
        let inst =
            gfx_backend_vulkan::Instance::create(name, version).expect("unsupported backend");
        let surf = unsafe {
            inst.create_surface(window)
                .expect("couldn't initialize surface")
        };
        (inst, surf)
    }
}
...
impl InstSurface
    for (
        ...
        <gfx_backend_dx11::Backend as Backend>::Surface,
    )
{
...
impl InstSurface
    for (
        ...
        <gfx_backend_metal::Backend as Backend>::Surface,
    )
{
...
pub struct DeviceInit<IS: InstSurface>(
    pub IS,
    pub Adapter<IS::Back>,
    pub <IS::Back as Backend>::Device,
    pub QueueGroup<IS::Back>,
);

pub fn init_device<IS: InstSurface>(
    window: &Window,
    adapter_selection: Option<usize>,
) -> Result<DeviceInit<IS>, Error> {
    init_helper(IS::create(WINDOW_NAME, 1, window), adapter_selection)
}
fn init_helper<IS: InstSurface>(
    inst_surface: IS,
    adapter_selection: Option<usize>,
) -> Result<DeviceInit<IS>, Error> {
    ...
    let mut gpu = unsafe {
        adapter
            ...
            .map_err(|e| {
                error!("{}", e);
                "Couldn't open the PhysicalDevice!"
            })?
    };
...
pub fn init_gl(
    ...
    ev: &EventLoop<()>,
    adapter_selection: Option<usize>,
) -> Result<(DeviceInit<gfx_backend_gl::Instance>, Window), Error> {
    let ctx = gfx_backend_gl::glutin::ContextBuilder::new()
        .build_windowed(window_builder, ev)
    ...
    let di = init_helper(
        gfx_backend_gl::Instance::Surface(surface),
        adapter_selection,
    )?;
    Ok((di, window))
}
