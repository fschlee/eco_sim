...
impl<B: Backend> DescriptorLayoutSpec<B> {
    pub fn new(
        textures: Vec<(usize, ShaderStageFlags)>,
        buffers: Vec<(gfx_hal::pso::DescriptorType, ShaderStageFlags)>,
    ) -> Self {
        Self {
            textures,
            buffers,
    ...
    pub fn build_layout(
        &self,
        device: &Dev<B>,
    ) -> Result<<B as Backend>::DescriptorSetLayout, Error> {
        let mut acc = Vec::new();
        ...
        unsafe {
            device.create_descriptor_set_layout(&acc, &[]).map_err(|e| {
                error!("{}", e);
                "out of memory".into()
            })
        }
    ...
    pub fn write_to_set(
        ...
        for &i in texture_indices {
            write_vec.push(gfx_hal::pso::DescriptorSetWrite {
                set: written_set,
                binding: 2 * i as u32,
                array_offset: 0,
        ...
        for &i in buffer_indices {
            ...
            write_vec.push(gfx_hal::pso::DescriptorSetWrite {
                ...
                descriptors: Some(gfx_hal::pso::Descriptor::Buffer(
                    buffers[i].buffer.deref(),
                    None..None,
                )),
            });
        }
        unsafe { device.write_descriptor_sets(write_vec) }
    }
...
impl<B: Backend> DescriptorPoolManager<B> {
    pub fn new(
        ...
        let pool = unsafe {
            device.create_descriptor_pool(
                pool_size,
                spec.create_pool_range(pool_size),
                gfx_hal::pso::DescriptorPoolCreateFlags::empty(),
            )
        }
        .map_err(|e| {
            error!("{}", e);
            "not enough memory for descriptor pool"
        })?;
        Ok(Self {
        ...
        unsafe {
            let mut new = device
                .create_descriptor_pool(
                    self.pool_size,
                    self.spec.create_pool_range(self.pool_size),
                    gfx_hal::pso::DescriptorPoolCreateFlags::empty(),
                )
                .map_err(|e| {
                    error!("{}", e);
    ...
    pub fn get_desc(&self, id: Id<Desc>) -> Result<&<B as Backend>::DescriptorSet, Error> {
        self.descriptor_sets
            .get(id.id as usize)
            .ok_or("descriptor set not registered".into())
    }
